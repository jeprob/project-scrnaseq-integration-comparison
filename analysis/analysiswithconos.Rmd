...
```
We find that there are 15 clusters identified in total. 
...
```
Many interesting things can be observed in this plot. The number of cells belonging to the clusters has a big range from many 1000 to less than 10 cells. Cells from smaller clusters still seemed to be observed in most samples, except for cluster 12 which does not appear in het1, cluster 14 which does not appear in p2 and cluster 15 which only appears inko2 and wt1. We also find that most of the cells in ko1 belong to cluster 2. Furthermore, there is no obvious trend, judged by eye, that wildtype sample cells seemed to be differently distributed than the remaining samples het1, m2 and p2. 
